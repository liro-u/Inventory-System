...
export const addItemToUser = async (req, res) => {
    ...
    try {
        ...
        const itemData = await Item.findOne(
            {_id: itemId},
            ["maxQuantityPerSlot", "maxSlot"]
        ).exec();

        // Retrieve the existing number of items.
        const userItem = await UserItem.findOne({userId, itemId}, ["quantity"]).exec()
        ...
        if (itemData.maxSlot > -1 && itemData.maxQuantityPerSlot > -1 && existingQuantity + quantity > itemData.maxSlot * itemData.maxQuantityPerSlot) {
            remainingQuantity = existingQuantity + quantity - itemData.maxSlot * itemData.maxQuantityPerSlot;
        ...
        res.status(200).json({userItem: userItemUpdated, remainingQuantity: remainingQuantity});
    } catch (error) {
        res
            .status(500)
